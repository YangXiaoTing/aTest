  ...
  <div>
      ...
      <div class="clearBoth">
        <div class="tableTr" v-for="(value,index) in banlanceData" :key="value.Address">
          ...
          <div class="tableTd">
            ...
            <button class="copyBtn ps-static" @click="copy(value.Address)" data-clipboard-action="copy" data-clipboard-target="span">复制</button>
          </div>
          <div class="tableTd">{{value.Balance | toFixed2}}</div>
          <div class="tableTd w-150">
            <button @click="openWithdrawalsBox(value,'showWithdrawalsBox')" v-show="value.isWithdrawal === true">提现</button>
      ...
      <div class="f-s-24 m-t-100">
        ...
        <div class="financingDropBox">
          ...
          <div class="dropDownBox" :style="{ background: theme.color}" v-show="isOpenDropDownList">
            <div class="dropDownItem" @click="chooseItem(value)" v-for="(value,index) in filterItem" :key="value.condition" v-text="value.condition"></div>
          </div>
    ...
    <div class="financingModal" v-show="showBox.showWithdrawalsBox===true">
      <div class="financingContent">
        ...
        <img class="el-icon-close" :src="require('@/assets/img/modalClose.png')" @click="closeMessageBox('showWithdrawalsBox')">
    ...
    <div class="financingModal" v-show="showBox.showExchangeBox===true">
      <div class="financingContent">
        <!--<i class="el-icon-close" @click="closeMessageBox('showWithdrawalsBox')"></i>-->
        <img class="el-icon-close" :src="require('@/assets/img/modalClose.png')" @click="closeMessageBox('showExchangeBox')">
        <div class="financingContent-wrap">
                ...
                <div class="dropDownList">
                  <el-dropdown @command="chooseCoin" trigger="click">
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item v-for="(value,index) in coinList" :command="value.coin" :key="index" v-text="value.coin"></el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </div>
              </div>
      ...
      async init () {
        ...
        if (res.code === 'StatusOk' && res.data instanceof Array) {
          res.data.forEach((item) => {
            coinList[this.coinSort[item.CoinName].sortIndex] = this.coinSort[item.CoinName]
            coinList[this.coinSort[item.CoinName].sortIndex].CoinName = item.CoinName
            coinList[this.coinSort[item.CoinName].sortIndex].Address = item.Address
            coinList[this.coinSort[item.CoinName].sortIndex].Balance = item.Balance
          })
