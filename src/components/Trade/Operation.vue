          ...
          <div class="infinite-table">
            ...
            <table cellspacing="0px" class="keeping-table">
              ...
              <tr v-for="item in keepingOrders"
                ...
                <td class="buy-text"><span v-text="((currentPrice/item.p-1)*100).toFixed(1)"></span>%</td>
            ...
            <infinite-loading :on-infinite="GetShareOrder"
        ...
        <div class="part-inner order-log">
          ...
          <div>
            ...
            <div v-if="currentOrderLogTab==='agency'" class="infinite-table">
              <table cellspacing="0px" id="agencyTable">
                ...
                <tbody>
                <tr v-for="(item,idx) in agencyTableData" :key="item.OrderId">
              ...
              <infinite-loading :on-infinite="GetSellShareOrder"
                                v-if="isLogged !== 'checking'"
                                spinner="waveDots"
                                ref="infiniteAgency">
                <span slot="no-more"></span>
          ...
          <div>
            ...
            <div v-if="currentOrderLogTab==='tran'" class="infinite-table">
              ...
              <infinite-loading :on-infinite="GetClosingVTS"
                                v-if="isLogged !== 'checking'"
                                spinner="waveDots"
  ...
  export default {
    ...
    methods: {
      ...
      async GetShareOrder () {
        ...
        let infiniteStatus = res.code === 'StatusOk' && this.pageIndex.keepingOrders < res.data.total && res.data.total !== 0 ? 'loaded' : 'complete'
        this.$refs.infiniteKeepingOrders.$emit(`$InfiniteLoading:${infiniteStatus}`)
      },
      async GetSellShareOrder () {
        if (!this.requestDone.agency) {
          let res = await this.$store.dispatch('infiniteGet', {
            invokeType: 'GetEntrustedVTS',
            pageIndex: this.pageIndex.agency++
          })
          let infiniteStatus = res.code === 'StatusOk' && this.pageIndex.agency < res.data.total && res.data.total !== 0 ? 'loaded' : 'complete'
          this.$refs.infiniteAgency.$emit(`$InfiniteLoading:${infiniteStatus}`)
          if (infiniteStatus === 'complete') {
            this.requestDone.agency = true
          }
        } else {
          this.$refs.infiniteAgency.$emit(`$InfiniteLoading:complete`)
        }
      },
      async GetClosingVTS () {
        if (!this.requestDone.ClosingVTS) {
          ...
          let infiniteStatus = res.code === 'StatusOk' && this.pageIndex.ClosingVTS < res.data.total && res.data.total !== 0 ? 'loaded' : 'complete'
          this.$refs.infiniteGetClosingVTS.$emit(`$InfiniteLoading:${infiniteStatus}`)
          if (infiniteStatus === 'complete') {
            this.requestDone.ClosingVTS = true
          }
        }
